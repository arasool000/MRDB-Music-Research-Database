...
$sql = "select A.artist_name,T.track_name,G.genre_name,Al.album_name,T.year,T.link from Artist A join Tracks T join Genre G join Album al on A.id=T.id AND T.genre_id=G.genre_id AND Al.album_id=T.album_id where T.track_name like '%$song1%'  ";
...
$sql1 = "update tracks set count=count+1 where track_name like '%$song1%';"; 
 $conn->query($sql1);
$conn->close();
?>
